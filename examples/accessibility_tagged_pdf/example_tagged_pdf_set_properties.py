...
def run_all_examples(data_dir=None, license_path=None):
    """Run examples of setting Structure Element properties and report status.
    Args:
    ...
    examples = [
        (
            "set_properties",
            set_properties,
            [path.join(output_dir, "set_properties.pdf")],
        ),
        (
            "set_text_elements",
            set_text_elements,
            [path.join(output_dir, "set_text_elements.pdf")],
        ),
        (
            "set_text_block_elements",
            set_text_block_elements,
            [path.join(output_dir, "set_text_block_elements.pdf")],
        ),
        (
            "set_inline_elements",
            set_inline_elements,
            [path.join(output_dir, "set_inline_elements.pdf")],
        ),
        ("set_tag_name", set_tag_name, [path.join(output_dir, "set_tag_name.pdf")]),
        (
            "set_elements",
            set_elements,
            [
                path.join(input_dir, "logo.jpg"),
                path.join(output_dir, "set_elements.pdf"),
            ],
        ),
        (
            "add_link_element",
            add_link_element,
            [path.join(output_dir, "add_link_element.pdf")],
        ),
        (
            "set_note_element",
            set_note_element,
            [path.join(output_dir, "set_note_element.pdf")],
        ),
        (
            "set_language_and_title",
            set_language_and_title,
            [path.join(output_dir, "set_language_and_title.pdf")],
        ),
    ]
